...
Flutter에서는 `Image`구성 요소를 통해 이미지를 로드하고 표시 `Image`할 수 있으며 데이터 소스는 자산, 파일, 메모리 및 네트워크가 될 수 있습니다.

1. 이미지 제공자
   `ImageProvider` 주로 이미지 데이터 획들을 위한 인터페이스를 정의하는 추상 클래스로 `load()`, 다른 데이터 소스에서 이미지를 가져오기 위해서는 다른 구현이 필요합니다. 예를 들어 Assets `ImageProvider`에서 이미지를 로드하는 ImageProvider가 구현되고 네트워크에서 이미지를 로드하는 ImageProvider가 구현 됩니다.

2. 이미지

`Image`위젯에는 이미지 제공자에 해당하는 필수 매개변수가 있습니다. 아래에서는 assets와 네트워크에서 각각 이미지를 로드하는 방법을 보여줍니다.
...
`fit` 효과 이미지
![](https://book.flutterchina.club/assets/img/3-12.3ae1737a.png)
...
![](https://book.flutterchina.club/assets/img/3-13.feb336de.png)
