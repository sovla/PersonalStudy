void readDeliveryExcel() async {
  try {
    ...
    var excelFile = pickedFile.files
        .where((element) => element.name.contains("xlsx"))
        .first;
    ...
    SpreadsheetTable table = decoder.tables["양식"] ??
        decoder.tables["시트1"] ??
    ...
    if (table.rows[1][2] == "상품명") {
      ...
      for (var i = 4; i < table.rows.length; i++) {
        ...
        var findImageStream = pickedFile.files.firstWhereOrNull((element) =>
            element.name.imageSplit(".").first ==
            mainImageUrls.imageSplit(".").first);

        if (findImageStream != null) {
          goods.mainImageUrls
              .add(makeImageUrl('goods', 'title0', findImageStream.name));
        } else {
        ...
        if (addImageUrls.isNotEmpty) {
          for (var i = 0; i < addImageUrls.split(",").length; i++) {
            ...
            var findImageStream = pickedFile.files.firstWhereOrNull((element) =>
                element.name.imageSplit(".").first ==
                file.imageSplit(".").first);
            if (findImageStream != null) {
              goods.mainImageUrls.add(
                  makeImageUrl('goods', 'title${i + 1}', findImageStream.name));
            } else {
              throw Exception(
                  "오류발생 올바르지 않은 추가이미지 입니다.\n$file 이미지가 존재하지 않습니다.\n상품명 : ${row[2]}");
            }
          }
        }
        ...
        for (var file in bodyImagesUrls.split(",")) {
          if (file.isNotEmpty) {
            ...
            if (findImageStream.isNotEmpty) {
              bodyImage.imageUrl = makeImageUrl(
                  'goods', 'body$index', findImageStream.first.name);

              goods.bodyImages.add(bodyImage);
            } else {
              throw Exception(
                  "오류발생 올바르지 않은 상세이미지입니다.\n$file 이미지가 존재하지 않습니다.\n상품명 : ${row[2]}");
            }
          }
          index++;
        }
        ...
        goods.kindId =
            row[18] == "일반상품" ? GoodKind.goods : GoodKind.none; // 상품 종류: 일반 상품
        ...
        if (row[21] != null && row[19] == "선택형") {
          ...
          for (var i = 0; i < selectTitle.split(",").length; i++) {
            String _title =
                selectTitle.split(",")[i].trim(); // title 에 공백이 생기는 경우가 있음
            int _price = int.parse(selectPrice.split(",")[i]);
            int _count = int.parse(selectCount.split(",")[i]);
            goodOptionItemList.add(GoodOptionItem(
                title: _title, addPrice: _price, hasCount: _count));
          }
        ...
        var _category = categories
            .where((element) =>
                ...
                _excelMainCategoryName.replaceAll(" ", ""))
            .where((element) {
          return element.subCategories
              .where((subCategory) =>
                  subCategory.name.replaceAll(" ", "") ==
                  _excelSubCategoryName.replaceAll(" ", ""))
              .isNotEmpty;
        });
      ...
      if (!isTest) {
        for (var reqData in reqDataList) {
          ...
          Iterable<ModelShoppingCategory> _category = reqData["category"];
          var _categoryFirst = _category.first;
          ...
          await RestAPI()
            ...
            if (goodsResponse.errorCode == 0) {
              _categoryFirst.subCategories =
                  _categoryFirst.subCategories.map((dbSubCategory) {
                return dbSubCategory;
              }).toList();

              for (var i = 0; i < _category.first.subCategories.length; i++) {
                var dbSubCategory = _category.first.subCategories[i];
                for (var excelSubCategory in row[1].split(",")) {
                  if (dbSubCategory.name.replaceAll(" ", "") ==
                      excelSubCategory.replaceAll(" ", "")) {
                    _category.first.subCategories[i].linkedGoodsIds
                        .add(goodsResponse.goodId!);
                  }
                }
              }
              ...
              for (var i = 0; i < goods.mainImageUrls.length; i++) {
                ...
                var fileName =
                    i == 0 ? mainImageUrls : addImageUrls.split(",")[i - 1];
                fileName = fileName.trim();
                fileName = fileName.contains(".")
                    ? fileName.imageSplit(".").first
                    : fileName;
                ...
                var alerdyImage = streamReadFileList.isNotEmpty
                    ? streamReadFileList
                        .where(
                          (item) =>
                              item != null &&
                              item.fileName == fileName.imageSplit(".").first,
                        )
                        .toList()
                    : null;
                debugPrint("메인이미지 ${i + 1}번째 넣음, $fileName");

                if ((alerdyImage == null || alerdyImage.isEmpty) &&
                    findImage.bytes == null) {
                  List<int> _bytes = [];
                  await for (var _byte in findImage.readStream!) {
                    _bytes.addAll(_byte);
                  }

                  uploadFiles.add(dio.MultipartFile.fromBytes(_bytes,
                      filename: _mainImage));
                  streamReadFileList
                      .add(AdminFileStream(fileName: fileName, bytes: _bytes));
                } else {
              ...
              RestAPI().adminShoppingCategoryUpdate(
                  AdminShoppingCategoryUpdateRequest(
